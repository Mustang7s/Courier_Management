...
if (!isset($_SESSION['courname'])) {
    header("Location: index.html");
}else{
  $filter = $_SESSION['courname'];
...
<head>
  ...
  <title>Courier Management System - Courier Homepage</title>
...
$sql = "SELECT `Product_ID`, `Name`, `Details`, `Image`, `Quantity`, `Price` FROM `product` WHERE `User_ID` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
$sql = "SELECT `Order_ID`, `User_ID`, `Supplier_ID`, `Product_ID`, `Total_Price`, `Quantity`, `Status` FROM `order` WHERE `Supplier_ID` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure you want to accept this order?')?window.location.href='insertion.inc.php?action=updateO&id=<?php echo($row["Order_ID"]); ?>&id1=<?php echo($row["Status"]); ?>':true;" title='Accept Order'>Accept</button></td>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure you want to complete this order?')?window.location.href='insertion.inc.php?action=updateO1&id=<?php echo($row["Order_ID"]); ?>&id1=<?php echo($row["Status"]); ?>':true;" title='Complete Order'>Complete</button></td>
</tr>
<?php
}
} else { echo "No results"; }
...
<div class="info_form ">
            <form action="insertion.inc.php" method="POST" enctype="multipart/form-data">
              <input type="text" placeholder="Enter product name" required  name="pname" />
              <br>
              <br>
              <input type="text" placeholder="Enter product details" required  name="pdet" />
              <br>
              <br>              
              <input type="number" placeholder="Enter product quantity" required  name="pquan" />
              <br>
              <br>                            
              <input type="number" placeholder="Enter product price" required  name="pprice" />
              <br>
              <br>
              <input type="file" accept=".jpg, .jpeg, .png" required  name="image" />                           
      <div class="btn-box">
        <button href="" name="addp" type="submit">
          Add
        </button>
      </div>
            </form>
...
<div class="info_form ">
            <form action="insertion.inc.php" method="POST" enctype="multipart/form-data">
                <select name="pid" required>
                                <option selected disabled value="0">Select A Product ID</option>
                                     <?php
                                      $con = mysqli_connect("localhost","root","","courier");
                                      $sql = "SELECT * FROM `product` WHERE `User_ID` = '$filter'";
                                      $all_categories = mysqli_query($con,$sql);
                                      while ($category = mysqli_fetch_array(
                                              $all_categories,MYSQLI_ASSOC)):;
                                  ?>
                                  <option value="<?php echo $category["Product_ID"];?>"><?php echo $category["Product_ID"];?></option>
                                  <?php
                                      endwhile;
                                  ?>
                </select>
              ...
              <br>                         
      <div class="btn-box">
        <button href="" name="upp" type="submit">
          Update
        </button>
      </div>
            </form>
...
<div class="info_form ">
            <form action="insertion.inc.php" method="POST">
              ...
              <br>                          
      <div class="btn-box">
        <button href="" name="regu" type="submit">
          Register
        </button>
      </div>
            </form>
