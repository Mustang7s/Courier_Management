...
<head>
  ...
  <title>Courier Management System - Administrator Homepage</title>
...
$sql = "SELECT `Product_ID`, `Name`, `Details`, `Image`, `Quantity`, `Price` FROM `product`";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
$sql = "SELECT `Order_ID`, `User_ID`, `Supplier_ID`, `Product_ID`, `Total_Price`, `Quantity`, `Status` FROM `order`";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
